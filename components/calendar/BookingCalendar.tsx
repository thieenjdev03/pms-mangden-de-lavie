...
export function mapBookingsToEvents(bookings: Booking[]): EventInput[] {
  return bookings
    .filter((b) => b.status !== "cancelled")
    .map((b) => {
      const room = rooms.find((r) => r.id === b.roomId);
      const roomLabel = room?.name ?? b.roomId;
      const propertyName = room?.property ?? "";
      const accent = getCalendarEventAccent(propertyName);
      return {
        id: b.id,
        ...
        extendedProps: { booking: b, roomLabel, propertyName, accent },
      };
    });
}
